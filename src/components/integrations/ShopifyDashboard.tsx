...
import type { ShopifySettings, ScanResult } from '../../types/integrations';
...
export function ShopifyDashboard({ settings }: ShopifyDashboardProps) {
  ...
  const startScan = async () => {
    setScanning(true);
    try {
      const result = await shopifyAPI.startScan(settings);
      if (result.success) {
  ...
  const pollScanResults = async (scanId: string) => {
    try {
      const results = await shopifyAPI.getScanResults(scanId);
      if (results) {
        setLastScan(results);
        setScanning(false);
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-sm text-gray-500">
                  Current Theme: {settings.theme.name}
                </p>
              </div>
            </div>
            <button
